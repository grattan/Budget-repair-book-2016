...
\GrattanReportNumber{2016-8}
\definecolor{DarkOrange}{HTML}{F68B33}
\acknowledgements{
...
\DeclareInstance{enotez-list}{grattanEndnotes}{paragraph}
 {
  heading=\subsection*{#1},
  notes-sep=0.5\baselineskip,
  format=\noindent\footnotesize,
  number=\makebox[0pt][r]{\textbf{#1.\ }},
 }
%\let\footnote=\endnote

\newcommand{\stopifendnote}[1]{}

\newcommand\todo[1]{\marginpar{\textbf{\textcolor{red}{#1}}}}
%% Pushes full-column floats to the right column
\makeatletter
\g@addto@macro\@floatplacement{%
\if@firstcolumn
% no \global according to David Carlisle
\@fpmin\textheight
\fi
}
\makeatother
...
\newenvironment{recommendations}[1][]%
  {\onecolumn\setkomafont{headsepline}{\color{DarkOrange}}\setkomafont{footsepline}{\color{DarkOrange}}\vtop to 0pt\bgroup\vspace{#1}\chapter*{\textcolor{DarkOrange}{Recommendations}}\begin{multicols}{2}}%
  {\end{multicols}\vss\egroup\hfill\twocolumn\setkomafont{headsepline}{\color{Orange}}\setkomafont{footsepline}{\color{Orange}}}

%% Goal: space between top figure and text should be stretched 
%% before parskip
% \setlength{\textfloatsep}{1\baselineskip plus 8.2\baselineskip minus 0.2\baselineskip}

% Subsubsections will be numbered
...
\begin{overview}[-15pt] %\addcontentsline{toc}{chapter}{Overview}
...
\begin{figure}[htbp]
\captionwithunits{Individuals earn most capital gains through real estate and shares}{Taxable net capital gains, 2013-14}\label{fig:CGT-by-entity-asset}
...
\notes{See \Vref{tbl:EMTR-savings-assumptions}.}
...
\caption{Assumptions for figure \Vref{fig:EMTR-savings}}\label{tbl:EMTR-savings-assumptions}
\small
...
% $-$\$0 & \$2,367,161,951 & 750,916 & 37.5\% \\ 
  ...
  \$50,000 & \$43,317,917 & 8,622 & 0.4\% \\ 
   \bottomrule
\end{tabularx}
\notes{Assumes that all net rental losses above the cap in a year cannot be written off against wage and salary income. That is, if the investor does not make other (positive) investment income in that year then the loss in excess of the cap cannot be claimed.}
...
The budget impact will depend on the level of the cap, as set out in \Cref{tbl:cap-NG}. A cap on losses of \$20,000 or more would affect very few landlords, but would make very little difference to the budget bottom line.
...
As well as raising more than \$6~billion a year, the changes will reduce (although not eliminate) distortions in investment choices toward debt financing of investment. Effective tax rates will increase a bit more for highly leveraged investors because they lose some of the tax benefits of upfront loss write offs (\Vref{fig:16}). However, effective tax rates will remain lower for this group because they still eventually write off their losses at a tax rate higher than they pay on gains.  
\begin{figure}
