...
fig35 %>%
  ...
  theme(strip.background = element_rect(colour = NA, fill = NA), 
        ...
        plot.margin = grid::unit(c(0.0, 0.1, 0.5, 0), "lines"),
...
dev.copy2pdf(file = "../b5-super-atlas/Figure3-5orig-portrait.pdf", width = 11, height = 7)
...
fig36p <-
  ...
  geom_blank(aes(x = Percentile, y = expenditure * 1.03)) + 
  scale_y_continuous("Annual expenditure (2015 dollars)", 
                     label = grattan_dollar, breaks = seq(0, 120e3, by = 20e3), expand = c(0,0))
fig36p
dev.copy2pdf(file = "../b5-super-atlas/Figure3-6orig.pdf", width = 11, height = 7)

fig36_with_legend <- fig36p + theme(legend.title = element_blank(), legend.position = "right",
...
grid.newpage()
grid.draw(g_legend(fig36_with_legend))
# dev.copy2pdf(file = "fig3-6-legend.pdf", width = 2.5, height = 4)
