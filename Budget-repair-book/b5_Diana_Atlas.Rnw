...
grplot <- function(...) grattan::grplot(...) + theme_hugh(base_family = "helvet", base_size = 23) + theme(axis.line.x = element_line(), axis.line.y = element_line())
@
...
<<FISCAL-atlas>>=
knitr::opts_chunk$set(fig.path = "./b5-diana-figure/")
@
...
<<property>>=
knitr::opts_chunk$set(fig.path = "./Property-taxes/b5-diana-atlas/figure/")
@
...
% SUPER
<<Super-part>>=
knitr::opts_chunk$set(fig.path = "./Super-tax-targeting/b5-diana-super-atlas/")
@
\newpage
...
net_transfers89 <- 
  ...
  mutate(Age = factor(Age, levels = unique(.$Age), ordered = TRUE)) %>%
  gather(fy_year, average_net_benefit, -Age) %>%
  filter(fy_year == "1988-89", complete.cases(.))

net_transfers09 <- 
  read_excel("./Super-tax-targeting/super-atlas/FIgure1-1.xlsx") %>%
  mutate(Age = factor(Age, levels = unique(.$Age), ordered = TRUE)) %>%
  gather(fy_year, average_net_benefit, -Age) %>%
  filter(fy_year == "2009-10", complete.cases(.))

net_transfers14 <- 
  net_transfers09 %>% mutate(fy_year = "2013-14")
net_transfers19 <- 
  net_transfers09 %>% mutate(fy_year = "2018-19")
net_transfers84 <- 
  net_transfers89 %>% mutate(fy_year = "1983-84")
net_transfers79 <- 
  net_transfers89 %>% mutate(fy_year = "1978-79")
...
read_excel("./Super-tax-targeting/super-atlas/FIgure1-1.xlsx") %>%
  ...
  gather(fy_year, average_net_benefit, -Age) %>%
  filter(complete.cases(.)) %>%
  {rbindlist(list(net_transfers79, net_transfers84, ., net_transfers14, net_transfers19), use.names = TRUE, fill = TRUE)} %>%
  mutate(age_contemp_num = as.numeric(sub("^.{0,1}([0-9]{2}).*$", "\\1", as.character(Age))), 
         age_contemp_num = ifelse(as.character(Age) == "<25", 15, age_contemp_num),
         year = fy2yr(fy_year), 
         year = ifelse(year == 2010, 2009, year),
         dob = year - age_contemp_num) %>%
  bind_rows("actual" = ., "offset" = ., .id = "cohort_offset") %>%
  mutate(dob = ifelse(cohort_offset == "actual", dob, dob + 5)) %>%
  arrange(dob) %>%
  group_by(dob) %>%
  # filter(min(age_contemp_num) == 15) %>%
  summarise(tot = sum(average_net_benefit), n()) %>%
  grplot(aes(x = dob, y = tot)) + 
  geom_area()
@
...
read_excel("./Super-tax-targeting/super-atlas/FIgure1-1.xlsx") %>%
  ...
  filter(year > 1999) %>%
  group_by(dob) %>%
  summarise(tot = mean(average_net_benefit)) %>%
  grplot(aes(x = dob, y = tot)) + 
  geom_area()
@
...
<<Figure3-and-5, out.height="11in", fig.height=11,eval=FALSE>>=
                       ...
                       ordered = TRUE)) %>%
{
  grplot(data = filter(., fill != "Current"), aes(x = Percentile, y = y, fill = fill)) + 
    ...
    scale_x_continuous(expand = c(0.0375,0)) + 
    geom_hline(yintercept = 0) + 
...
<<CGT>>=
knitr::opts_chunk$set(fig.path = "./Hot-property-CG-and-NG/CGT-NG-atlas/b5-diana-atlas/")
@
...
<<CGT-by-age-income, fig.height=7.125, out.height="7.125in", fig.width=12.1, out.width="12.1in">>=
sample_file %>%
  mutate(
  ...
  mutate(
    ...
    text.y = mean(probCG[`Taxable income percentile` > 96]) -0.02 + ifelse(`Age group` == "Below\n30", 
                                                                     0.01, 0)
         ) %>%
         {
           grplot(., aes(x = `Taxable income percentile`, y = probCG, 
             ...
             coord_cartesian(xlim = c(-3, 115), ylim=c(0,0.26)) +
...
<<CG-marginal-tax-rates-delayed, out.height="7.125in", fig.height=7.125>>=
...
<<CGT-Net-rent-time-bar-chart>>=
individuals_table1_201314 %>%
  #filter(grepl("Net.rent...((loss)|(profit))$", `Selected items`), unit == "$") %>%
  filter(grepl("Net rent2", Selected_items)) %>%
  select(-Count) %>%
  filter(!is.na(Sum)) %>%
  mutate(real_value = cpi_inflator(Sum, from_fy = fy_year, to_fy = "2014-15")) %>%
  ...
  scale_y_continuous(label = function(x){paste0(grattan_dollar(x / 1e9, digits = 1), "\u2006bn")}) + 
  theme(plot.margin = grid::unit(c(0.7,0.7,0.5,0.00), "lines"), 
        axis.title.x = element_blank())
@

<<NG-PG-by-age, fig.width=12.65, out.width="12.65in", fig.height=8.0, out.height="8.0in">>=
sample_file %>%
...
<<EMTR-by-gearing>>=
structure(list(gearing_ratio = c(0, 0.4, 0.8), emtr = c(0.40611931339248, 
0.293001549752088, 0.188448961525236), `Gearing ratio` = structure(1:3, .Label = c("0%", 
"40%", "80%"), class = "factor")), class = "data.frame", .Names = c("gearing_ratio", 
"emtr", "Gearing ratio"), row.names = c(NA, -3L)) %>% 
