...
grattanCharts::setup_knitr_for_grattex()
# font.add("helvet", 
...
grplot <- function(...) grattanCharts::grplot(...) + 
...
", sep = "\t", header = TRUE, as.is = TRUE, check.names = FALSE, na.strings = "") %>%
  ...
  mutate(y = as.numeric(gsub("%", "", y, fixed = TRUE))) %>%
  {
    grplot(., aes(x = Year, y = y, fill = fill), reverse = TRUE) + 
      ...
      geom_text(data = data_frame(Year = 2017.5, y = 4,
                                  fill = factor("Underlying\ncash\nbalance", levels = levels(.$fill))), 
...
if (file.exists("./Fiscal-challenges/Figure10.xlsx"))
