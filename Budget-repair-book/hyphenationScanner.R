system("pdftotext -layout b5_Diana_book.pdf")
hyphens <- pdf_as_text <- readLines("b5_Diana_book.txt")
library(data.table)
library(dplyr)
...
#
Diana_log <- readLines("b5_Diana_book.log")
Diana_source <- readLines("b5_Diana_book.tex")
Super_source <- readLines("./Super-tax-targeting/b5-Diana-portrait-Super-tax-targeting-as-chapter.tex")
GST_source <- readLines("./GST-reform-package/b5-Diana-portrait-GST-reform-package-as-chapter.tex")
CGT_source <- readLines("./Hot-property-CG-and-NG/b5-Dianna-CGT_and_neg_gearing_final-as-chapter.tex")
...
overfulls %>%
  ...
  merge(data.table(srcno = 0:5, src = c("base", "Super", "GST", "CGT", "back", "back2"))) %>%
  filter(points != 10.95, between(points, 1.5, 30)) %>% # chapters
